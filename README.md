...
## Автономная исследовательская версия

Добавлен отдельный пользовательский маршрут без ЛИРА-САПР и SCAD. Первая
версия принимает вручную заданные данные только одной прямоугольной балки и
запускает существующий кратковременный детерминированный рабочий процесс:
...
Маршрут формирует локальный отчёт и ZIP, но сохраняет
`project_use=false`, `requires_engineer_review=true`. Числа и подобранные схемы
в отчёте имеют только диагностический статус и не являются утверждённой
расчётной способностью или проектным решением. Порядок Windows-установки и
точные ограничения приведены в `docs/standalone/`.

Windows ZIP является сетевым пакетом для рецензирования, а не офлайн- или
самодостаточным дистрибутивом: первая установка требует интернета для загрузки
фиксированных зависимостей Python 3.11. Существующий подбор арматуры в этом
...
В окне доступны встроенная сводка проверенного публичного ZIP, условная схема
введённого сечения и отдельные сообщения. Сводка не публикует изгибную несущую
способность или внутренние промежуточные значения, а схема не является рабочим
чертежом либо фактической раскладкой арматуры.
