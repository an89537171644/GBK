...
from sp63_core.ml import build_report_neural_surrogate_result
...
def test_report_neural_surrogate_builds_from_jsonl(tmp_path):
    dataset_path = _write_batch_dataset(tmp_path)

    result = build_report_neural_surrogate_result(
    ...
    assert result.model_name == "MLPClassifier"
    ...
    assert any("not be used as a design checker" in warning for warning in result.warnings)


def test_report_neural_surrogate_builds_from_csv(tmp_path):
...
def test_report_neural_surrogate_input_only_excludes_leakage_columns(tmp_path):
    dataset_path = _write_batch_dataset(tmp_path)
...
def test_report_neural_surrogate_deterministic_derived_features_warn(tmp_path):
    dataset_path = _write_batch_dataset(tmp_path)
...
def test_report_neural_surrogate_small_dataset_requires_review(tmp_path):
    dataset_path = _write_batch_dataset(tmp_path)
...
def test_report_neural_surrogate_missing_target_fails(tmp_path):
    dataset_path = _write_batch_dataset(tmp_path)
    ...
    result = build_report_neural_surrogate_result(dataset_path=missing_path)
...
def test_report_neural_surrogate_constant_target_requires_review(tmp_path):
    dataset_path = _write_batch_dataset(tmp_path)
    rows = _read_jsonl(dataset_path)
    for row in rows:
    ...
    result = build_report_neural_surrogate_result(dataset_path=constant_path)
...
def test_cli_report_neural_surrogate_json_output(tmp_path, capsys):
    dataset_path = _write_batch_dataset(tmp_path)
    capsys.readouterr()
    ...
    assert exit_code == 0
    assert payload["command"] == "report-neural-surrogate"
