...
| `workflows/engineering_workflow_batch.py` | implemented batch engineering workflow runner over input JSON folders with case summaries and static batch index | yes | yes | use only as batch review orchestration; failed cases require engineer review |
...
| `workflows/protected_files_guard.py` | implemented protected files guard for formula/material/external validation release checks | yes | yes | use as review aid only; guard does not approve merge or project use |
