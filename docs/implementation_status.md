...
| `workflows/gui_planning.py` | implemented historical K64 planning decision with CLI-first static report recommendation | yes | yes | Issue #134 supersedes only the Tkinter postponement for the narrow standalone Windows beam trial; broader decision remains historical |
| `standalone/gui_logic.py` | implemented headless form parsing, status presentation, unique output paths, and fail-closed result-link gates for the narrow Windows trial | yes | yes | presentation/safety adapter only; existing standalone controller remains authoritative and project use remains prohibited |
| `standalone/gui.py` | implemented Russian Tkinter engineer interface for the standalone rectangular-beam Windows research trial | yes | yes | manual Windows UAT and engineer review remain required; no formulas, broader elements, ML, or project approval are added |
| `standalone/package.py` | implemented integrity-recorded Windows package and `02_OPEN_GBK.cmd` GUI launcher with real hidden-Tk-window smoke gate | yes | yes | use only as research-trial packaging; built artifact must pass Windows Actions and user acceptance |
...
| `workflows/engineering_handoff_package.py` | implemented portable engineering handoff package with inputs, evidence templates, docs, preview, commands, and SHA256 manifest | yes | yes | use as scaffold only; engineer must fill evidence and review deterministic outputs |
| `workflows/launcher_scripts.py` | implemented lightweight launcher scripts package for CLI workflow wrappers | yes | yes | use as command convenience only; scripts do not implement calculations or UI |
| `workflows/external_validation_evidence_package.py` | implemented external validation evidence package with templates, checklist, optional CSV summary, and SHA256 manifest | yes | yes | engineer must provide real external values; package does not approve project use |
| `workflows/v09_final_audit.py` | implemented aggregated v0.9 final audit across protected files, docs, readiness, demo, handoff, launcher, material, and external evidence checks | yes | yes | use as release-preparation evidence only; audit does not publish or certify release |
| `workflows/v09_review_closure.py` | implemented v0.9 manual review closure and release-candidate stabilization report | yes | yes | use as manual review evidence only; material, external validation, signoff, and project approval gates remain separate |
| `workflows/v09_release_candidate_package.py` | implemented final v0.9 release candidate package with start-here README, manifest, ZIP, and compact evidence artifacts | yes | yes | use as engineering review evidence only; package does not publish a release or approve project use |
| `workflows/v09_package_verify.py` | implemented v0.9 release candidate package verification with ZIP, forbidden-file, manifest, and manual acceptance log outputs | yes | yes | use as package verification evidence only; manual review remains required and project use stays false |
| `workflows/agent_sprint_guard.py` | implemented local K-step artifact completeness guard for agent sprint continuation | yes | yes | use as local guard only; GitHub PR/Issue state still requires separate verification |
| `workflows/release_notes.py` | implemented v0.9 release notes, checklist, and known-limitations package | yes | yes | documentation only; does not publish release or approve project use |
